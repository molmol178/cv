...
void pen__bit1_draw(uchar **data,gcRec *cg,long x,long y)
{
  data[y][x] = 1;
}
...
void pen__make(Edit e)
{
  ...
  switch (e->pen.shape)
    {
    case SHAPE_BOX:
      cgfillbox(Image.data(e->pen.pat),e->pen.bit1_pen,0,0,size-1,size-1);
      break;
    case SHAPE_CIRCLE:
      cgfillcircle4(Image.data(e->pen.pat),e->pen.bit1_pen,0,0,size-1,size-1);
      break;
    }
}
...
void pen__draw(char **data,gcRec *cg,long x,long y)
{
  ...
  for (iy = 0; iy < size; iy++)
      ...
      for (ix = 0; ix < size; ix++)
	{
	  ...
	  switch (type)
	    {
	    case Bit1: pen__draw_bit1((bit1 **)data,px,py);break;
	    case UChar: pen__draw_uchar((uchar **)data,px,py);break;
	    case UChar3: pen__draw_uchar3((uchar3 **)data,px,py); break;
	    }
	}
    }
}
