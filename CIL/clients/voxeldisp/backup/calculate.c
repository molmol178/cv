  ...
  _P1 (( t_environment *, environment ))
{
  ...
  for ( z = 0; z < zsize; z++ )
      ...
      for ( x = 0; x < xsize; x++ )
	{
	  ...
	  if ( data[z][y][x] && (!(data[z-1][y][x] && data[z+1][y][x] &&
  ...
  for ( i = 0; i < 6; i++ )
    {
      ...
      environment->strength_list[i] = strength;
  ...
  _P1 (( t_environment *, environment ))
{
  ...
  range = Voxel.zsize( environment->object ) * environment->range;
  for ( i = 0; i < nplane; i++ )
    {
